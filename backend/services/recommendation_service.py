...
GENERAL_BDE_FALLBACK_BASE_SCORE = 5
GENERAL_BDE_FALLBACK_BROAD_PROFILE_BOOST = 8
GENERAL_BDE_FALLBACK_CURRENT_SEMESTER_BOOST = 3
GENERAL_BDE_FALLBACK_PREFERENCE_BOOST = 12
GENERAL_BDE_FALLBACK_MPE_LISTED_PENALTY = -6
GENERAL_BDE_FALLBACK_SAME_FACULTY_PENALTY = -10
GENERAL_BDE_FALLBACK_OTHER_FACULTY_BOOST = 4
...
def recommend_courses(
    ...
    sorted_recommendations = add_general_bde_fallback_recommendations(
        db=db,
        choice_slots=candidate_slots,
        recommendations=sorted_recommendations,
        preferred_tags=preferred_tags,
        normalized_student_faculty=normalized_student_faculty,
        completed_codes=completed_codes,
...
def add_general_bde_fallback_recommendations(
    db: Session,
    choice_slots: list[RecommendationChoiceSlot],
    recommendations: list[CourseRecommendation],
    preferred_tags: set[str],
    normalized_student_faculty: Optional[str],
    completed_codes: set[str],
    excluded_codes: set[str],
    excluded_titles: set[str],
    curriculum_courses: list[RecommendationCurriculumCourse],
    ...
    used_slot_ids = {
        ...
        for recommendation in recommendations
        if recommendation.matchedChoiceSlotId
    }
    unfilled_bde_slots = [
        slot
        for slot in get_assignment_ordered_choice_slots(choice_slots)
        if normalize_choice_slot_code(slot.courseCode) == "BDE" and
        slot.slotId not in used_slot_ids
    ]

    if not unfilled_bde_slots:
        return recommendations

    fallback_levels = {
        level
        for slot in unfilled_bde_slots
        for level in get_general_bde_candidate_levels(slot.year)
    }
    query = db.query(ModuleModel).filter(ModuleModel.code.is_not(None))

    if fallback_levels:
        query = query.filter(ModuleModel.level.in_(fallback_levels))

    modules = query.order_by(ModuleModel.code).all()
    prerequisites_by_module = get_prerequisites_by_module(db, [module.code for module in modules])
    unlocks_by_module = get_unlocks_by_module(db, [module.code for module in modules])
    prerequisite_modules_by_code = get_prerequisite_modules_by_code(db, prerequisites_by_module)
    fallback_recommendations_by_slot: dict[str, list[CourseRecommendation]] = {}
    already_used_codes = {
        recommendation.courseCode.upper()
        for recommendation in recommendations
    }
    already_used_titles = set(excluded_titles)

    for recommendation in recommendations:
        add_used_title_keys(already_used_titles, recommendation.title)

    blocked_codes = completed_codes | excluded_codes | already_used_codes

    for module in modules:
        if is_excluded_module(module, blocked_codes, already_used_titles):
            continue
        ...
        for slot in unfilled_bde_slots:
            if not can_module_fit_general_bde_slot(module, slot, normalized_student_faculty):
                continue
            ...
            fallback_score = get_general_bde_fallback_score(
                module,
                preferred_tags,
                readiness,
            ...
            if fallback_score < GENERAL_BDE_FALLBACK_BASE_SCORE:
                continue
            ...
            fallback_recommendations_by_slot.setdefault(get_choice_slot_identity(slot), []).append(
                CourseRecommendation(
                    courseCode=module.code,
                    title=module.title,
                    academicUnits=module.au,
                    ...
                    level=module.level,
                    mpeSpecialisations=module.mpe_specialisations or [],
                    matchedChoiceSlot="BDE",
                    matchedChoiceSlotId=slot.slotId,
                    matchedChoiceSlotYear=slot.year,
                    matchedChoiceSlotSemester=slot.semester,
                    matchedKeywords=get_general_bde_fallback_signals(module, preferred_tags),
                    prerequisites=prerequisites,
                    missingPrerequisites=readiness.missing_prerequisites,
                    existingPrerequisiteCourseCodes=readiness.existing_prerequisite_course_codes,
                    plannedPrerequisiteCourseCodes=readiness.planned_prerequisite_course_codes,
                    ...
                    recommendationConfidence="general",
                    scoreBreakdown=RecommendationScoreBreakdown(
                        careerTagScore=0,
                        careerSkillScore=0,
                        careerSkillEvidence=None,
                        currentSemesterBonus=(
                            GENERAL_BDE_FALLBACK_CURRENT_SEMESTER_BOOST
                            if module.is_current_semester
                            else 0
                        ),
                        preferenceBoost=get_general_bde_preference_boost(module, preferred_tags),
                        sameFacultyBoost=get_general_bde_faculty_adjustment(
                            module,
                            normalized_student_faculty,
                        ),
                        mpeSpecialisationBoost=GENERAL_BDE_FALLBACK_MPE_LISTED_PENALTY
                        if module.mpe_specialisations
                        else 0,
                        legacyCodePenalty=0,
                        defaultProfileAdjustment=GENERAL_BDE_FALLBACK_BROAD_PROFILE_BOOST
                        if module.recommendation_profile == "broad-default"
                        else 0,
                        prerequisitePlanningPenalty=readiness.prerequisite_planning_penalty,
                        unlockContribution=get_unlock_contribution(readiness.unlock_value),
                        finalScore=fallback_score,
                    ),
                    reason=build_general_bde_fallback_reason(module, preferred_tags),
                )
            )

    return append_ranked_general_bde_fallbacks(
        recommendations,
        fallback_recommendations_by_slot,
        unfilled_bde_slots,
        preferred_tags,
        limit,
    )

def get_general_bde_fallback_score(
    module: ModuleModel,
    preferred_tags: set[str],
    readiness: RecommendationReadiness,
    normalized_student_faculty: Optional[str],
) -> int:
    score = GENERAL_BDE_FALLBACK_BASE_SCORE

    if module.is_current_semester:
        score += GENERAL_BDE_FALLBACK_CURRENT_SEMESTER_BOOST

    if module.recommendation_profile == "broad-default":
        score += GENERAL_BDE_FALLBACK_BROAD_PROFILE_BOOST

    score += get_general_bde_preference_boost(module, preferred_tags)
    score += get_general_bde_faculty_adjustment(module, normalized_student_faculty)
    score += get_unlock_contribution(readiness.unlock_value)

    if module.mpe_specialisations:
        score += GENERAL_BDE_FALLBACK_MPE_LISTED_PENALTY

    score += readiness.prerequisite_planning_penalty

    return max(1, score)

def get_general_bde_candidate_levels(year: Optional[int]) -> list[int]:
    if year is None:
        return [1, 2, 3, 4]

    return list(range(1, get_preferred_bde_level(year) + 1))

def can_module_fit_general_bde_slot(
    module: ModuleModel,
    slot: RecommendationChoiceSlot,
    student_programme: Optional[str],
) -> bool:
    ...
    if slot.year is None or module.level is None:
        return True

    return module.level <= get_preferred_bde_level(slot.year)

def get_general_bde_faculty_adjustment(
    module: ModuleModel,
    normalized_student_faculty: Optional[str],
) -> int:
    if not normalized_student_faculty or not module.faculty:
        return 0

    if module.faculty.upper() == normalized_student_faculty:
        return GENERAL_BDE_FALLBACK_SAME_FACULTY_PENALTY

    return GENERAL_BDE_FALLBACK_OTHER_FACULTY_BOOST

def get_general_bde_preference_boost(module: ModuleModel, preferred_tags: set[str]) -> int:
    ...
    return (
        GENERAL_BDE_FALLBACK_PREFERENCE_BOOST
        if module_tags.intersection(preferred_tags)
        else 0
    )

def get_general_bde_fallback_signals(module: ModuleModel, preferred_tags: set[str]) -> list[str]:
    matched_preference_tags = sorted(set(module.recommendation_tags or []).intersection(preferred_tags))

    return ["general-bde", *[f"tag:{tag}" for tag in matched_preference_tags]]

def build_general_bde_fallback_reason(module: ModuleModel, preferred_tags: set[str]) -> str:
    matched_preference_tags = sorted(set(module.recommendation_tags or []).intersection(preferred_tags))

    if matched_preference_tags:
        return (
            "General BDE suggestion. This module fits an open BDE slot and matches "
            f"selected topic preference(s): {', '.join(matched_preference_tags)}."
        )

    return (
        "General BDE suggestion. This module fits an open BDE slot, but it is not "
        "a strong career-specific recommendation."
    )

def append_ranked_general_bde_fallbacks(
    recommendations: list[CourseRecommendation],
    fallback_recommendations_by_slot: dict[str, list[CourseRecommendation]],
    unfilled_bde_slots: list[RecommendationChoiceSlot],
    preferred_tags: set[str],
    limit: int,
) -> list[CourseRecommendation]:
    ...
    for recommendation in ranked_recommendations:
        add_used_title_keys(used_course_titles, recommendation.title)
        add_used_recommendation_tags(used_recommendation_tags, recommendation)

    for slot in unfilled_bde_slots:
        if len(ranked_recommendations) >= limit:
            break

        slot_recommendations = sorted(
            fallback_recommendations_by_slot.get(get_choice_slot_identity(slot), []),
            key=lambda recommendation: (
                -recommendation.score,
                bool(recommendation.mpeSpecialisations),
                recommendation.courseCode,
            ),
        )
        fallback_recommendation = get_unique_recommendation_at_or_after_index(
        ...
        ranked_recommendations.append(fallback_recommendation)
        used_course_codes.add(fallback_recommendation.courseCode)
        add_used_title_keys(used_course_titles, fallback_recommendation.title)
        add_used_recommendation_tags(used_recommendation_tags, fallback_recommendation)
