...
- Backend exposes health, roadmap, transcript upload, curriculum guide upload, module catalog, faculty, and recommendation endpoints.
...
- Recommendations are generated by `POST /recommendations` using the module catalog and the student's completed module codes.
- The current supported career goal is `Software Engineer`.
- Backend scoring uses weighted keyword matches such as `software`, `programming`, `database`, `web`, `cloud`, `systems`, `algorithm`, `testing`, and `security`.
