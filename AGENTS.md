...
- For CSC profiles, legacy `CZ` course codes are deprioritized behind current `SC` course codes, but remain fallback candidates if no better current-code option is available.
